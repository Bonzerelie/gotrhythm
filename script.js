/* =========================
   ...
   (() => {
    ...
    // Capture window around the player's bar (half-beat before + half-beat after)
    const CAPTURE_EARLY_BEATS = 0.5;
    const CAPTURE_LATE_BEATS = 0.5;
  
    // Scoring tolerances (ms). Smaller = stricter.
    const SCORING = {
      MATCH_MAX_MS: 180,
      TIER_5_MS: 55,
      TIER_4_MS: 90,
    ...
    // Metronome buffers are now REQUIRED for metronome playback (no synthesized fallback)
    ...
    // Hits that will be scored for the *current* round (only within captureWindow)
    let roundHits = []; // {tSec, i}
  
    // Capture window (computed in READY so early taps before PLAY can still be caught)
    let captureWindow = null; // { startSec, endSec }
    let playBarStartSec = 0; // convenience
    ...
    function playMetronomeClick(whenSec, isDownbeat) {
      // metronome must come from audio files only; no synthesized fallback
    ...
    kickBtn.addEventListener("click", () => registerHit("K"));
    snareBtn.addEventListener("click", () => registerHit("S"));
